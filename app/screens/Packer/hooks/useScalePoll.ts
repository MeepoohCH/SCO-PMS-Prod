...
export function useScalePoll({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/scale-verifications?production_detail_id=${lot.id}`)
        if (!res.ok) return
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const list = await res.json() as any[]
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const anyApproved = Array.isArray(list) && list.some((v: any) => v.is_locked === true || v.pl_approved_at !== null)
        if (anyApproved) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (r1Poll) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const v1Approved = list.some((v: any) => v.round_no === 1 && (v.is_locked === true || v.pl_approved_at !== null))
          if (v1Approved) {
        ...
        if (r2Poll) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const v2Approved = list.some((v: any) => v.round_no === 2 && (v.is_locked === true || v.pl_approved_at !== null))
          if (v2Approved) {
    ...
    return () => clearInterval(interval)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [latexScale1Pending, latexScale1Approved, latexScale2Pending, latexScale2Approved, lot.id])
}