  ...
  try {
    ...
    if (!resolvedProductId && product_name?.trim()) {
      ...
      if (existingProduct) {
        ...
        console.log(
          "[POST /api/lots] auto-created product:",
          newProduct.id,
          product_name,
        );
      }
    ...
    const safeProductId = resolvedProductId
      ? await validFk("products", resolvedProductId)
      : null;
    const safeCustomerId = customer_id
      ? await validFk("customers", customer_id)
      : null;
    const safePackagingTypeId = packaging_type_id
    ...
    try {
      ...
      lot = await prisma.production_details.create({
        data: {
          ...
          ...(safeCustomerId && {
            customer: { connect: { id: safeCustomerId } },
          }),
          ...(safePackagingTypeId && {
            packaging_type: { connect: { id: safePackagingTypeId } },
          }),
          ...(target_amount_mt != null && { target_amount_mt }),
          ...(planned_pallets != null && {
            planned_pallets: Number(planned_pallets),
          }),
          ...
          ...(resolvedOperationDate && {
            operation_date: new Date(resolvedOperationDate),
          }),
